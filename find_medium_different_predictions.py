...
def main():
    print("Loading GPT-2 medium with TransformerLens...")
    model = HookedTransformer.from_pretrained("gpt2-medium", device="cuda")
    
    # Load all example pairs
    with open("gpt2_medium_example_pairs.json", 'r') as f:
    ...
    if good_pairs:
        # Save the good pairs
        with open("gpt2_medium_different_prediction_pairs.json", 'w') as f:
            json.dump(good_pairs, f, indent=2)
        print("Saved to gpt2_medium_different_prediction_pairs.json")
    else:
